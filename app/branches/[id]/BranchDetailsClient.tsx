...
const BranchDetailsClient = ({ branch, allRequests }: BranchDetailsClientProps) => {
  ...
  // Loading State එක පාලනයට Row Index එක භාවිත කරයි
  ...
  const handleRequest = async (item: any, type: "ACCOUNT" | "LOAN" | "INVESTMENT", index: number) => {
    // Document Number එක පමණක් ලබා ගැනීම (නැතහොත් හිස් string එකක්)
    const docNo = item.account_number || item.acc_no || item.accountNo || item.contract_no || item.loan_no || item.inv_no || item.contractNo || "";
    
    // Loading indicator එක සඳහා Unique key එකක් ලෙස Type එක සහ Index එක භාවිත කරයි
    ...
    setIsSubmitting(loadingKey);
    try {
      const response = await fetch('/api/document-request', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          docNumber: docNo, // docNo පමණක් යවයි (හිස් නම් "")
          documentType: type,
          branchId: branch.id,
        }),
      });
      ...
      if (response.ok) {
        // Index එක මගින් UI එකෙන් Item එක ඉවත් කිරීම (id කිසිසේත්ම භාවිත නොවේ)
        if (type === "ACCOUNT") {
              ...
              <tbody className="divide-y divide-slate-100">
                {accounts && accounts.length > 0 ? (
                  accounts.map((acc: any, index: number) => {
                    ...
                    const loadingKey = `ACCOUNT-${index}`;
                    return (
                      <tr key={index} className="hover:bg-slate-50 transition-colors">
                        <td className="px-6 py-5 font-bold text-slate-700">{docNo}</td>
                        <td className="px-6 py-5 text-right">
                          <button
                            onClick={() => handleRequest(acc, "ACCOUNT", index)}
                            disabled={isSubmitting === loadingKey}
                          ...
                          >
                            {isSubmitting === loadingKey ? <Loader2 size={12} className="animate-spin" /> : "Submit"}
                          </button>
                        </td>
                      </tr>
                    );
              ...
              <tbody className="divide-y divide-slate-100">
                {loans && loans.length > 0 ? (
                  loans.map((loan: any, index: number) => {
                    ...
                    const loadingKey = `LOAN-${index}`;
                    return (
                      <tr key={index} className="hover:bg-slate-50 transition-colors">
                        <td className="px-6 py-5 font-bold text-slate-700">{docNo}</td>
                        <td className="px-6 py-5 text-right">
                          <button
                            onClick={() => handleRequest(loan, "LOAN", index)}
                            disabled={isSubmitting === loadingKey}
              ...
              <tbody className="divide-y divide-slate-100">
                {investments && investments.length > 0 ? (
                  investments.map((inv: any, index: number) => {
                    ...
                    const loadingKey = `INVESTMENT-${index}`;
                    return (
                      <tr key={index} className="hover:bg-slate-50 transition-colors">
                        <td className="px-6 py-5 font-bold text-slate-700">{docNo}</td>
                        <td className="px-6 py-5 text-right">
                          <button
                            onClick={() => handleRequest(inv, "INVESTMENT", index)}
                            disabled={isSubmitting === loadingKey}
