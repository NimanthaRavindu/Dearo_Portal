...
export default async function AccountPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function addAccount(formData: FormData) {
    ...
    // Hidden input එකෙන් ලැබෙන Base64 String එක ලබා ගැනීම
    const base64Data = formData.get("billPhotoBase64") as string;

    // Database එකට Record එක Save කිරීම
    await prisma.account.create({
      data: {
        ...
        branchId: branchId,
        billPhoto: base64Data || null, // Base64 Data URL එක direct save වේ
      }
    });
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        ...
        <form action={addAccount} className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          {/* Hidden Fields for Base64 Data & File Name */}
